...
import type { AvailableTimeSlot, BookableService, ServiceProfessional } from "@/scheduling/application/dtos/PublicBookingDtos";
...
const CalendarToolbar = ({ label, onNavigate }: ToolbarProps<CalendarAppointment>) => <div className="tirr__calendar-toolbar">
  <div className="tirr__calendar-toolbar__navigation">
    ...
    <button type="button" onClick={() => onNavigate("NEXT")} aria-label="Proximo periodo" title="Proximo periodo"><Icon name="chevron-right" /></button>
  </div>
  <strong>{label}</strong>
...
const AgendaPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = view === "day"
        ...
        : view === "month"
          ? await application.administration.appointments.month(businessId, date.getFullYear(), date.getMonth() + 1)
          : await application.administration.appointments.week(businessId, dateOnly(moment(date).startOf("week").toDate()));
  ...
  const openBooking = async () => { setDrawer(true); setBooking(emptyBooking); setServices([]); setProfessionals([]); setSlots([]); setBookingResourcesLoading(true); try { setServices(await application.booking.listServices.execute({ businessId })); } catch (caught) { setError(caught instanceof Error ? caught.message : "Nao foi possivel carregar os servicos."); } finally { setBookingResourcesLoading(false); } };
  const selectService = async (serviceId: string) => { setBooking({ ...booking, serviceId, professionalId: "", startsAtUtc: "" }); setProfessionals([]); setSlots([]); if (!serviceId) return; setBookingResourcesLoading(true); try { setProfessionals(await application.booking.listProfessionals.execute({ businessId, serviceId })); } catch (caught) { setError(caught instanceof Error ? caught.message : "Nao foi possivel carregar os profissionais."); } finally { setBookingResourcesLoading(false); } };
  const selectDate = async (value: string, professionalId = booking.professionalId) => { setBooking((current) => ({ ...current, date: value, startsAtUtc: "" })); setSlots([]); if (!booking.serviceId || !professionalId || !value) return; setBookingResourcesLoading(true); try { setSlots(await application.booking.listAvailableTimeSlots.execute({ businessId, professionalId, serviceId: booking.serviceId, date: value })); } catch (caught) { setError(caught instanceof Error ? caught.message : "Nao foi possivel carregar os horarios."); } finally { setBookingResourcesLoading(false); } };
