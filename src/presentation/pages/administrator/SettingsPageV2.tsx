...
const SettingsPageV2 = () => {
  ...
  const profile = useApiData<BusinessProfile | null>(() => application.administration.overview.profile(businessId), businessId, null);
  const hours = useApiData<OperatingHoursDay[]>(() => application.administration.overview.operatingHours(businessId), businessId, []);
  const keys = useApiData<ApiKeyItem[]>(() => permissions.includes("api_keys.get") ? application.identity.execute({ type: "listApiKeys", businessId }) : Promise.resolve([]), `${businessId}:${permissions.join("|")}`, []);
  ...
  const saveAccount = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); try { const updated = await application.authentication.execute({ type: "updateProfile", fullName: account.fullName, email: account.email }); useAuthStore.setState({ user: updated }); setMessage("Perfil atualizado."); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Nao foi possivel salvar."); } finally { setBusy(false); } };
  ...
  const createKey = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); try { const created = await application.identity.execute({ type: "createApiKey", businessId, name: keyForm.name.trim(), permissions: keyForm.permissions, expiresAtUtc: keyForm.expiresAtUtc ? new Date(keyForm.expiresAtUtc).toISOString() : null }); setSecret(created); setDrawer(null); setKeyForm({ name: "", expiresAtUtc: "", permissions: [] }); await keys.reload(); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Nao foi possivel criar."); } finally { setBusy(false); } };
  const rotate = async (item: ApiKeyItem) => { if (!await confirm({ title: `Rotacionar ${item.name}?`, description: "A chave atual deixara de funcionar e um novo segredo sera gerado.", confirmLabel: "Rotacionar", tone: "primary" })) return; setSecret(await application.identity.execute({ type: "rotateApiKey", businessId, id: item.id })); await keys.reload(); };
  const revoke = async (item: ApiKeyItem) => { if (!await confirm({ title: `Revogar ${item.name}?`, description: "Integracoes que usam esta chave perderao o acesso imediatamente.", confirmLabel: "Revogar" })) return; await application.identity.execute({ type: "revokeApiKey", businessId, id: item.id }); await keys.reload(); };
  ...
  return <div className="tirr__admin__page"><section className="tirr__admin__panel"><div className="tirr__admin__panel-header"><div><p className="tirr__admin__overline">Preferencias e acesso</p><h2>Configuracoes</h2></div>{tab === "integrations" && permissions.includes("api_keys.post") && <button className="btn btn-primary" onClick={() => setDrawer("key")}><Icon name="plus-lg" /> Nova chave</button>}</div><AdminTabs value={tab} onChange={setTab} items={[{ value: "business", label: "Estabelecimento", icon: "building" }, { value: "security", label: "Conta e seguranca", icon: "shield-check" }, { value: "integrations", label: "Integracoes", icon: "key" }]} />{message && <div className="tirr__inline-alert">{message}</div>}
    {tab === "business" && <><PageFeedback loading={profile.loading || hours.loading} error={profile.error || hours.error} onRetry={() => void Promise.all([profile.reload(), hours.reload()])} />{!profile.loading && !profile.error && !profile.data && <AdminEmptyRow icon="building-x" description="Nao foi possivel encontrar os dados desta empresa.">Estabelecimento sem perfil</AdminEmptyRow>}{profile.data && <div className="tirr__settings-grid"><section><div className="tirr__section-heading"><h3>Dados da empresa</h3><div className="tirr__section-actions"><button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => void copySchedulerLink()}><Icon name={schedulerLinkCopied ? "check2" : "copy"} /> {schedulerLinkCopied ? "Link copiado" : "Copiar link da agenda"}</button>{permissions.includes("business.put") && <button className="tirr__admin__icon-button" onClick={openBusiness} title="Editar" aria-label="Editar dados da empresa"><Icon name="pencil" /></button>}</div></div><dl className="tirr__details-list"><div><dt>Nome</dt><dd>{profile.data.name}</dd></div><div><dt>Razao social</dt><dd>{profile.data.legalName || "Nao informada"}</dd></div><div><dt>Documento</dt><dd>{profile.data.documentNumber || "Nao informado"}</dd></div><div><dt>Slug</dt><dd>{profile.data.slug}</dd></div><div><dt>Fuso horario</dt><dd>{profile.data.timeZone}</dd></div><div><dt>Status</dt><dd><StatusPill active={profile.data.status === "active"} label={profile.data.status} /></dd></div></dl></section><section><div className="tirr__section-heading"><h3>Horario de funcionamento</h3>{permissions.includes("availability.put") && <button className="tirr__admin__icon-button" onClick={() => { setHoursForm(hours.data.map((item) => ({ ...item }))); setDrawer("hours"); }} title="Editar"><Icon name="pencil" /></button>}</div>{hours.data.length ? <div className="tirr__hours-list">{hours.data.map((item) => <div key={item.dayOfWeek}><strong>{DAY_LABEL[item.dayOfWeek]}</strong><span>{item.isOperating ? `${item.opensAt?.slice(0, 5)} - ${item.closesAt?.slice(0, 5)}` : "Fechado"}</span></div>)}</div> : <AdminEmptyRow icon="clock" description="Defina os dias e horarios em que o estabelecimento atende.">Horario de funcionamento nao configurado</AdminEmptyRow>}</section></div>}</>}
    ...
    {tab === "integrations" && <><PageFeedback loading={keys.loading} error={keys.error} onRetry={() => void keys.reload()} /><div className="tirr__admin__data-list">{apiKeys.map((item) => <article className="tirr__admin__data-row" key={item.id}><span className="tirr__admin__client-avatar"><Icon name="key" /></span><div className="tirr__admin__data-main"><h3>{item.name}</h3><p>{item.prefix}... · {item.permissions.length} permissoes</p></div><div><small>Ultimo uso</small><span>{item.lastUsedAtUtc ? new Date(item.lastUsedAtUtc).toLocaleString("pt-BR") : "Nunca"}</span></div><StatusPill active={item.isActive} /><div className="tirr__row-actions">{permissions.includes("api_keys.put") && item.isActive && <button className="tirr__admin__icon-button" onClick={() => void rotate(item)} title="Rotacionar"><Icon name="arrow-repeat" /></button>}{permissions.includes("api_keys.delete") && item.isActive && <button className="tirr__admin__icon-button is-danger" onClick={() => void revoke(item)} title="Revogar"><Icon name="slash-circle" /></button>}</div></article>)}{!keys.loading && !apiKeys.length && <AdminEmptyRow>Nenhuma chave criada.</AdminEmptyRow>}</div></>}
    </section>
