...
const AppointmentOperations = () => {
  ...
  const changeStatus = async (status: Appointment["status"], reason?: string) => {
    ...
    setMessage("");
    try {
      await application.administration.appointments.updateStatus(businessId, appointmentId, status, reason ?? undefined);
      await load();
  ...
  const reschedule = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setMessage("");
    try {
      await application.administration.appointments.reschedule(businessId, appointmentId, new Date(rescheduleAt).toISOString());
      setRescheduleAt("");
      await load();
