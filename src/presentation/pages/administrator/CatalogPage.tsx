...
const CatalogPage = () => {
  ...
  const servicesState = useApiData<ServiceSummary[]>(() => application.administration.overview.services(businessId), businessId, []);
  const categoriesState = useApiData<ServiceCategory[]>(() => application.administration.categories.list(businessId), businessId, []);
  ...
  const editService = async (summary: ServiceSummary) => {
    ...
    try { const item = await application.administration.services.get(businessId, summary.serviceId); setServiceForm({ id: item.id, serviceCategoryId: item.serviceCategoryId, name: item.name, description: item.description ?? "", durationInMinutes: item.durationInMinutes, price: item.price, isActive: item.isActive }); }
  ...
  const editCategory = async (summary: ServiceCategory) => {
    ...
    try { const item = await application.administration.categories.get(businessId, summary.id); setCategoryForm({ id: item.id, name: item.name, description: item.description ?? "", isActive: item.isActive }); }
  ...
  const saveService = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (serviceForm.id) await application.administration.services.update(businessId, serviceForm.id, input);
  ...
  const saveCategory = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (categoryForm.id) await application.administration.categories.update(businessId, categoryForm.id, input);
  ...
  const removeService = async (item: ServiceSummary) => { if (!await confirm({ title: `Excluir ${item.name}?`, description: "O servico sera removido do catalogo e deixara de aceitar novos agendamentos.", confirmLabel: "Excluir" })) return; try { await application.administration.services.remove(businessId, item.serviceId); await servicesState.reload(); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Nao foi possivel excluir."); } };
  const removeCategory = async (item: ServiceCategory) => { if (!await confirm({ title: `Excluir ${item.name}?`, description: "A categoria sera removida permanentemente do catalogo.", confirmLabel: "Excluir" })) return; try { await application.administration.categories.remove(businessId, item.id); await categoriesState.reload(); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Nao foi possivel excluir."); } };
