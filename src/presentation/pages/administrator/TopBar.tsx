...
const META = [
  {
    match: "/clientes",
    title: "Clientes",
    description: "Relacionamento, historico e dados de contato.",
  },
  {
    match: "/catalogo",
    title: "Catalogo",
    description: "Servicos, categorias, precos e duracoes.",
  },
  { match: "/equipe", title: "Equipe", description: "Profissionais e acessos ao estabelecimento." },
  {
    match: "/disponibilidade",
    title: "Disponibilidade",
    description: "Jornadas recorrentes e excecoes de horario.",
  },
  {
    match: "/configuracoes",
    title: "Configuracoes",
    description: "Estabelecimento, seguranca e integracoes.",
  },
] as const;

/**
 * @description Topbar administrativa com contexto da rota, empresa ativa, tema e sessão.
 *
...
const TopBar = () => {
  ...
  /**
   * @description Resolve os metadados de navegação correspondentes à rota administrativa atual.
   *
   * @returns Resultado produzido pela operação.
   */
  const page = useMemo(
    () =>
      META.find((item) => pathname.includes(item.match)) ?? {
        title: "Agenda",
        description: "Visualize horarios e acompanhe os atendimentos.",
      },
    [pathname],
  );
  ...
  useEffect(() => {
    ...
    const close = (event: MouseEvent | KeyboardEvent) => {
      ...
      if (event instanceof MouseEvent && !switcherRef.current?.contains(event.target as Node))
        setOpen(false);
    };
  ...
  /**
   * @description Troca o estabelecimento ativo e conduz o usuário para o painel correspondente.
   *
   * @param nextId - Valor de next id utilizado pela operação.
  ...
  return (
    <header className="tirr__topbar">
      <div className="tirr__topbar-title">
        <h1>{page.title}</h1>
          ...
          >
            ...
            <span>
              ...
              <strong>
                {isSwitching
                  ? "Trocando empresa..."
                  : (activeBusiness?.name ?? "Selecionar empresa")}
              </strong>
            </span>
            {businesses.length > 1 && <Icon name="chevron-down" />}
          </button>
          {open && businesses.length > 1 && (
            <div className="tirr__business-menu" role="menu">
              {businesses.map((item) => (
                <button
                  type="button"
                  key={item.businessId}
                  className={item.businessId === activeBusiness?.businessId ? "active" : ""}
                  onClick={() => void switchBusiness(item.businessId)}
                >
                  <span>
                    <strong>{item.name}</strong>
                    <small>{item.roles.join(" / ")}</small>
                  </span>
                  {item.businessId === activeBusiness?.businessId && <Icon name="check2" />}
                </button>
              ))}
            </div>
          )}
