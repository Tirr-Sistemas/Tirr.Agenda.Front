...
import { availabilityApi, professionalsApi } from "@/service/api";
import type { AvailabilityException, AvailabilityExceptionType, AvailabilityRule, Professional } from "@/service/api";
...
const AvailabilityPage = () => {
  ...
  const rules = useApiData<AvailabilityRule[]>(() => availabilityApi.rules(businessId, professionalFilter || undefined), `${businessId}:${professionalFilter}`, []);
  const exceptions = useApiData<AvailabilityException[]>(() => availabilityApi.exceptions(businessId, professionalFilter || undefined), `${businessId}:${professionalFilter}`, []);
  ...
  const editRule = async (summary: AvailabilityRule) => { setDrawer("rules"); setBusy(true); try { const item = await availabilityApi.rule(businessId, summary.id); setRuleForm({ ...item }); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Nao foi possivel carregar a regra."); } finally { setBusy(false); } };
  const editException = async (summary: AvailabilityException) => { setDrawer("exceptions"); setBusy(true); try { const item = await availabilityApi.exception(businessId, summary.id); setExceptionForm({ id: item.id, professionalId: item.professionalId, date: item.date, type: item.type, startTime: item.startTime?.slice(0, 5) ?? "", endTime: item.endTime?.slice(0, 5) ?? "", reason: item.reason ?? "" }); } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Nao foi possivel carregar a excecao."); } finally { setBusy(false); } };

  const saveRule = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setMessage("");
    if (ruleForm.startTime >= ruleForm.endTime) {
      setMessage("O horario final da regra deve ser posterior ao horario inicial.");
      return;
    }
    ...
    try {
      ...
      if (ruleForm.id) await availabilityApi.updateRule(businessId, ruleForm.id, input);
      else await availabilityApi.createRule(businessId, input);
      setDrawer(null);
      await rules.reload();
    } catch (caught) {
      setMessage(caught instanceof Error ? caught.message : "Nao foi possivel salvar a regra.");
    } finally {
      setBusy(false);
    }
  };
  const saveException = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setMessage("");
    if (Boolean(exceptionForm.startTime) !== Boolean(exceptionForm.endTime)) {
      setMessage("Informe o horario inicial e final, ou deixe ambos vazios para o dia inteiro.");
      return;
    }
    if (exceptionForm.startTime && exceptionForm.endTime && exceptionForm.startTime >= exceptionForm.endTime) {
      setMessage("O horario final da excecao deve ser posterior ao horario inicial.");
      return;
    }
